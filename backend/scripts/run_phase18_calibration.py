...
def generate_and_label_observations(ticks: list, spec: InstrumentSpecification) -> List[SignalResearchObservation]:
    ...
    for tick_idx, t in enumerate(ticks):
        ...
        if fused.direction != "NONE":
            ...
            # Extract future ticks for post-hoc outcome labeling
...
def run_phase18_benchmark():
    ...
    print("\n==================================================")
    print(" PHASE 18 SIGNAL OUTCOME DATASET & CALIBRATION")
    print("==================================================")

    for sym, ticks in raw_data.items():
        spec = InstrumentSpecification.get_default_spec(sym)
        print(f"\nProcessing & Post-Hoc Labeling Candidate Signals for {sym} ({len(ticks):,} ticks)...")
        obs_list = generate_and_label_observations(ticks, spec)
        all_observations.extend(obs_list)

        wf_calibrator = WalkForwardCalibrator()
        wf_report = wf_calibrator.run_split(obs_list)

        symbol_reports[sym] = {
            "total_candidate_signals": len(obs_list),
            "walk_forward_calibration": wf_report.to_dict()
        }

        print(f"  -> {sym}: Candidate Signals = {len(obs_list):,}")
        print(f"     Status: {wf_report.summary}")

    # Overall Combined Walk-Forward Calibration
    overall_wf = WalkForwardCalibrator().run_split(all_observations)

    report_output = {
        "total_research_observations": len(all_observations),
        "overall_walk_forward_calibration": overall_wf.to_dict(),
        "symbol_reports": symbol_reports
    }

    out_path = Path(__file__).resolve().parent / "phase18_calibration_report.json"
    ...
    print(f"[SUCCESS] Phase 18 Calibration Report saved to {out_path}")

if __name__ == "__main__":
    run_phase18_benchmark()
